...
all_table <- annotate_contam(all_table, all_blast, sample_names = methods, max_off = max_dist)
...
write_tsv(seq_stats, path = file.path(analysis_path, "zymo_neat_seq_stats.tsv"))
...
zymo_gg <- all_table_gg
save(zymo_gg, file = file.path(plot_path, "zymo_gg.RData"))
...
snr_box <- ggplot(data = all_table_gg, aes(x = method, y = log10_count)) +
...
snr_violin <- ggplot(data = all_table_gg, aes(x = method, y = log10_count)) +
...
snr_jitter <- ggplot(data = all_table_gg %>% filter(count > 0), aes(x = method, y = log10_count)) +
...
snr_bees <- ggplot(data = all_table_gg %>% filter(count > 0), aes(x = method, y = log10_count)) +
