...
result_path <- "~/thesis/results/kozich_130403"
ref_path <- "~/thesis/references"
analysis_path <- "~/thesis/analysis/kozich_130403"
...
ref_fasta <- "kozich_HMP_MOCK.fasta"

ref_seqs <- readDNAStringSet(file.path(ref_path, ref_fasta)) %>% as.character()
strains <- lapply(str_split(names(ref_seqs), "\\."), `[`, 1:2) 
strains <- sapply(strains, paste0, collapse = ".")
...
write_tsv(summary_table, path = file.path(analysis_path, "kozich_mock1_summary_table.tsv"))
...
write_tsv(seq_stats, path = file.path(analysis_path, "kozich_mock1_seq_stats.tsv"))
...
write_tsv(read_stats, path = file.path(analysis_path, "kozich_mock1_read_stats.tsv"))
...
kozich_gg <- all_table_gg
save(kozich_gg, file = file.path(plot_path, "kozich_gg.RData"))
...
snr_box <- ggplot(data = all_table_gg) +
  ...
  labs(title = "Distribution of log10(read counts) for reference and non-reference sequences",
          subtitle = "Kozich 'Mock1' mock community", x = "method", y = "log10(read counts)") +
...
comp_bar <- ggplot(all_table_gg, aes(x = method, y = count)) +
  ...
  labs(title = "Relative composition of sample processed by different methods", 
       subtitle = "Kozich 'Mock1' mock community",
...
snr_box_contam <- ggplot(all_table_gg %>% filter(!ref_like), aes(x = method, y = log10_count)) +
  ...
  labs(title = "Distribution of log10(read counts) for non-hit contaminant and non-contaminant sequences",
          subtitle = "Kozich 'Mock1' sample processed with various methods", x = "sample dilution", y = "log10(read counts)") +
