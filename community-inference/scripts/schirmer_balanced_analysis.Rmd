...
result_path <- "~/thesis/results/schirmer_balanced"
ref_path <- "~/thesis/references"
analysis_path <- "~/thesis/analysis/schirmer_balanced"
...
write_tsv(summary_table, path = file.path(analysis_path, "schirmer_balanced_summary_table.tsv"))
...
write_tsv(seq_stats, path = file.path(analysis_path, "schirmer_balanced_seq_stats.tsv"))
...
write_tsv(read_stats, path = file.path(analysis_path, "schirmer_balanced_read_stats.tsv"))
...
schirmer_gg <- all_table_gg
save(schirmer_gg, file = file.path(plot_path, "schirmer_gg.RData"))
...
snr_box <- ggplot(data = all_table_gg) +
  ...
  labs(title = "Distribution of log10(read counts) for reference and non-reference sequences",
          subtitle = "Schirmer balanced mock community", x = "method", y = "log10(read counts)") +
